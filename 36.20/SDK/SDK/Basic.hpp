...
namespace Offsets
{
	constexpr int32 GObjects          = 0x14E017B0;
	constexpr int32 AppendString      = 0x0184AB50;
	constexpr int32 GNames            = 0x15135300;
	constexpr int32 GWorld            = 0x1488D840;
	constexpr int32 ProcessEvent      = 0x01A33BF8;
...
// Predefined struct FUObjectItem// 0x0018 (0x0018 - 0x0000)
...
static_assert(sizeof(FUObjectItem) == 0x000014, "Wrong size on FUObjectItem");
...
public:
	...
	inline class UObject* GetByIndex(const int32 Index)
	{
		...
		return (UObject*)(8 * (Item->Field1 | ((unsigned __int64)(Item->Field2 & 0x3FFF) << 32)));
	}

    inline class UObject* GetObjectByIndex(const int32 Index)
    {
        auto Item = GetItemByIndex(Index);
...
{
public:
	uint8 Pad[0x800]; //just gona set the size
};
...
public:
	...
	{
		static auto FName__Ctor = (void (*)(FName*, const wchar_t*, int))(InSDKUtils::GetImageBase() + 0x1847508);
...
public:
	...
	FUtf8StrProperty_                             SubPathString;                                     // 0x0008(0x0010)(ZeroConstructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
};
...
public:
	...
	void Process(Args... args)
	{
		struct Params
		{
			...
			Params(Args&&... args) : Data(std::forward<Args>(args)...) {}
		} Arg(std::forward<Args>(args)...);

		for (int i = 0; i < InvocationList.Num(); i++)
		{
			auto& ScriptDelegate = InvocationList[i];
			if (auto Object = ScriptDelegate.Object.Get())
			{
				UClass* ObjClass = Object->Class;
				if (!ObjClass)
					continue;

				UFunction* Function = ObjClass->FindFunction(ScriptDelegate.FunctionName.ToString().c_str());
				if (Function)
				{
					Object->ProcessEvent(Function, &Arg);
				}
			}
		}
	}

	bool IsBoundToObject(UObject* InObject) const
	{
		for (const auto& ScriptDelegate : InvocationList)
		{
			if (ScriptDelegate.Object.Get() == InObject)
			{
				return true;
			}
		}
